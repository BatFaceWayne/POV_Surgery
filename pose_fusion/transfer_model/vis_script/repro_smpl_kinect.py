...
def_matrix = read_deformation_transfer(
    '/home/rui/projects/sp2_ws/smplx_transfer/transfer_data/smplx2smpl_deftrafo_setup.pkl')
def_matrix = def_matrix.float().to(device)
...
def main(args):
    ...
    if render is True:
        if args.rendering_mode == '3d' or args.rendering_mode == 'both':
            ...
            try:# transformation from master camera to scene mesh
    ...
    material_interacteenote = pyrender.MetallicRoughnessMaterial(
        metallicFactor=0.0,
        alphaMode='OPAQUE',
        baseColorFactor=(1.0, 1, 193 / 255, 1.0)  # pink, interactee
    )
    ...
    body_model_camera = build_layer(model_path, model_type='smpl',
                                    gender=camera_wearer_gender, ext='pkl', use_compressed=False)
    body_model_camera = body_model_camera.to(device)
    for i_frame in tqdm(range(start_frame_dict[args.recording_name], end_frame_dict[args.recording_name]+1)[args.start::args.step]):
        ...
        body1 = trimesh.load('/media/rui/My Passport/egobody_release/debug_transfer_SMPLX/original_smplx/'+ args.recording_name +'/interactee/'+frame_id[6:]+'.ply', process=False)
        body2 = trimesh.load('/media/rui/My Passport/egobody_release/debug_transfer_SMPLX/original_smplx/'+ args.recording_name +'/camera_wearer/'+frame_id[6:]+'.ply', process=False)
        #
        # body1 = trimesh.load('/media/rui/My Passport/egobody_release/debug_transfer_SMPLX/smpl_mesh/'+ args.recording_name +'/interactee/'+frame_id+'.ply', process=False)
        # body2 = trimesh.load('/media/rui/My Passport/egobody_release/debug_transfer_SMPLX/smpl_mesh/'+ args.recording_name +'/camera_wearer/'+frame_id+'.ply', process=False)


        if args.view != 'master':
            body1.apply_transform(trans_maintosub)
        if args.view != 'master':
            body2.apply_transform(trans_maintosub)
            # material_interacteenote
        body = trimesh.util.concatenate([body1, body2])
        # smplx_body1 = body1
        # smplx_body2 = body2
        if render is True:
            body_mesh_interactee1 = pyrender.Mesh.from_trimesh(body1, material=material_interacteenote)
            body_mesh_interactee = pyrender.Mesh.from_trimesh(body2, material=material_interactee)

        ##### read camera wearer smplx params

        # body = trimesh.load(
        #     '/media/rui/My Passport/egobody_release/debug_transfer_SMPLX/smpl_mesh/recording_20220215_S21_S22_01/camera_wearer/' + frame_id + '.ply',
        #     process=False)
        # body1 = trimesh.load('/media/rui/My Passport/egobody_release/debug_transfer_SMPLX/original_smplx/recording_20210907_S03_S04_01/interactee/'+frame_id[6:]+'.ply', process=False)
        # body2 = trimesh.load('/media/rui/My Passport/egobody_release/debug_transfer_SMPLX/original_smplx/recording_20210907_S03_S04_01/camera_wearer/'+frame_id[6:]+'.ply', process=False)

        # if not osp.exists(osp.join(fitting_root_interactee, 'body_idx_{}'.format(interactee_idx), 'results', frame_id, '000.pkl')):
        #     print('interactee fitting {} do not exist!'.format(holo_frame_id))
        #     continue
        # # osp.join(fitting_root_interactee, 'body_idx_{}'.format(interactee_idx), 'results', holo_frame_id, '000.pkl'
        ...
        for key in param.keys():
            if key in ['pose_embedding', 'camera_rotation', 'camera_translation', 'gender']:
                ...
                if key == 'betas' or key == 'transl':
                    torch_param[key] = torch.tensor(param[key]).float().to(device)
                elif key == 'body_pose':
                    torch_param[key] = torch.tensor(R.from_rotvec(param[key].reshape(23,3)).as_matrix()).float().unsqueeze(0).to(device)
                else:
                    torch_param[key] = torch.tensor(R.from_rotvec(param[key].reshape(1,3)).as_matrix()).unsqueeze(0).float().to(device)
        output = body_model_interatee(return_verts=True, **torch_param)
        vertices = output.vertices.detach().cpu().numpy().squeeze()

        # joints = output.joints.detach().cpu().numpy().squeeze()
        body1 = trimesh.Trimesh(vertices, body_model_interatee.faces, process=False)
        with open(
                osp.join(fitting_root_camera_wearer, 'body_idx_{}'.format(camera_wearer_idx), 'results', frame_id, '000.pkl'),
                'rb') as f:
            param = pickle.load(f)
            ...
            if key in ['pose_embedding', 'camera_rotation', 'camera_translation', 'gender']:
                ...
                if key == 'betas' or key == 'transl':
                    ...
                    torch_param[key] = torch.tensor(
                        R.from_rotvec(param[key].reshape(23, 3)).as_matrix()).float().unsqueeze(0).to(device)
                else:
                    torch_param[key] = torch.tensor(R.from_rotvec(param[key]).as_matrix()).unsqueeze(0).float().to(
                        device)
        output = body_model_camera(return_verts=True, **torch_param)
        vertices = output.vertices.detach().cpu().numpy().squeeze()


        body2 = trimesh.Trimesh(vertices, body_model_camera.faces, process=False)

        body = trimesh.util.concatenate([body1, body2])
        # body = trimesh.Trimesh(vertices, model_camera_wearer.faces, process=False)
        # criterion_shape(vertices,np.array(body2.vertices))
        # smplx_body1_vert = apply_deformation_transfer(def_matrix, torch.from_numpy(smplx_body1.vertices).unsqueeze(0).float().to(device),
        #                                 torch.from_numpy(smplx_body1.faces).unsqueeze(0).float().to(device))
        # smplx_body2_vert = apply_deformation_transfer(def_matrix, torch.from_numpy(smplx_body2.vertices).unsqueeze(0).float().to(device),
        #                                 torch.from_numpy(smplx_body2.faces).unsqueeze(0).float().to(device))
        # temp_1 = criterion_shape(smplx_body1_vert, output1.vertices)
        # temp_2 = criterion_shape(smplx_body2_vert, output2.vertices)
        # all_dict[fitting_root_camera_wearer.split('/')[-1] + '/'+frame_id] = [temp_1.item(),temp_2.item()]
        # if args.view != 'master':
        ...
        if render is True:
            ...
            if args.rendering_mode == 'body' or args.rendering_mode == 'both':
                ...
                scene.add(body_mesh_interactee1, 'body_mesh_interactee1')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--release_data_root', type=str, default='/home/rui/projects/virtual_human_ws/egobody_release')
    parser.add_argument('--recording_name', type=str, default='recording_20210907_S03_S04_01')
    ...
    for rec in os.listdir('/home/rui/projects/virtual_human_ws/egobody_release/smpl_interactee/'):
    # for rec in ['recording_20210907_S03_S04_01','recording_20220215_S21_S22_01']:
        args.recording_name = rec
        main(args)
    # with open('/home/rui/Downloads/v2v_loss_transfer_after.json', 'w') as fp:
