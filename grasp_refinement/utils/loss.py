...
def Contact_loss(obj_xyz, hand_xyz, cmap):
    ...
    if False:
        ...
        B = obj_xyz.size(0)
        # cmap = torch.ones((1,778))
        cmap = torch.ones([1, len(hand_xyz[0])], dtype=torch.bool)
        obj_CD, _ = utils_loss.get_NN(hand_xyz_prior,obj_xyz )  # [B, N1] NN distance from obj pc to hand pc
        n_points = torch.sum(cmap)
        cmap_loss = obj_CD[cmap].sum() / (B * n_points)
        return 1000.0 * cmap_loss
    else:
        prior_idx = f1 + f2 + f3 + f4 + f5 + f0
        hand_xyz_prior = hand_xyz  # [:, prior_idx, :]  # only using prior points for contact map
        B = obj_xyz.size(0)
...
def finger_contact_loss(obj_xyz, hand_xyz, cmap):
    '''
    # hand-centric loss, encouraging hand touching object surface
    :param obj_xyz: [B, N1, 3]
    :param hand_xyz: [B, N2, 3]
    ...
    obj_CD = torch.cdist(obj_xyz[0,:,:], hand_xyz_prior[0], p=2)
    # obj_CD
    # obj_CD, _ = utils_loss.get_NN(obj_xyz, hand_xyz_prior)  # [B, N1] NN distance from obj pc to hand pc
    # n_points = torch.sum(cmap)
    # cmap_loss = obj_CD[cmap].sum() / (B * n_points)
    return 40 * obj_CD.min()
...
def TTT_loss(hand_xyz, hand_face, obj_xyz, cmap_affordance, cmap_pointnet):
    ...
    finger_contact_loss1 = 2.5 * finger_contact_loss(obj_xyz, hand_xyz, cmap=nn_dist < 0.02**2)
    return penetr_dist, consistency_loss, contact_loss, finger_contact_loss1
