...
class HO3D(torch.utils.data.Dataset):
    def __init__(self, transform, data_split):
        ...
        if self.data_split == 'train':
            self.mode = 'train'
            self.base_info = pickle.load(open('/media/rui/mac_data/POV_surgery/2d_repro_ho3d_style_hocc_cleaned.pkl', 'rb'))
        else:
            self.mode = 'validation'
            self.base_info = pickle.load(open('/media/rui/mac_data/POV_surgery/2d_repro_ho3d_style_val_cleaned.pkl', 'rb'))
    ...
    def __getitem__(self, idx):
        ...
        # img = Image.open(os.path.join('/media/rui/T7/POV_surgery/color/', seqName, id + '.jpg')).convert("RGB")
        frame_anno = pickle.load(open(os.path.join('/media/rui/T7/POV_surgery/annotation', seqName, id + '.pkl'), 'rb'))
        ...
        # img
        joints_coord_img = joints_uv
        img = load_img(os.path.join('/media/rui/T7/POV_surgery/color/', seqName, id + '.jpg'))
        bbox = get_bbox(joints_coord_img[:, :2], np.ones_like(joints_coord_img[:, 0]), expansion_factor=1.5)
        bbox = process_bbox(bbox, img['width'], img['height'], expansion_factor=1.0)
        ...
        if self.data_split == 'train':
            ## 2D joint coordinate
            joints_img = joints_coord_img #data['joints_coord_img']
