...
class CoarseNet(nn.Module):
    ...
    def sample_poses(self, bps_object, seed=None):
        ...
        return self.decode(Zgen, bps_object), Zgen

    def sample_poses_near_prior(self, bps_object, prior =None, seed=None):
        bs = bps_object.shape[0]
        np.random.seed(seed)
        dtype = bps_object.dtype
        device = bps_object.device
        self.eval()
        prior = prior
        variance_list = np.random.normal(0., 1, size=(bs, self.latentD))
        empty_list = []
        # for temp_i in range(self.latentD):
        #     temp_list = np.random.uniform(-1, 1, 100)
        #     temp_list.sort()
        #     empty_list.append(temp_list.reshape(bs,1))
        # variance_list = np.concatenate(empty_list, 1)

        with torch.no_grad():
            Zgen = np.zeros((bs, self.latentD))

            for temp_i in range(bs):
                Zgen[temp_i,:] = prior + variance_list[temp_i,:]

            Zgen = torch.tensor(Zgen,dtype=dtype).to(device)

        return self.decode(Zgen, bps_object), Zgen

    def sample_poses_uniform(self, bps_object, seed=None):
        bs = bps_object.shape[0]
        np.random.seed(seed)
        dtype = bps_object.dtype
        device = bps_object.device
        self.eval()
        prior = np.array([[-0.3355331 , -1.03232366, -0.37435417, -0.12739535, -0.4291598 ,
        -0.867512  ,  1.30976358, -1.03916669,  0.11635853, -0.82445792,
         1.39367525, -0.27417302,  0.94194435,  1.05387509, -1.54951947,
         0.91382424]])
        variance_list = np.random.normal(0., 0.1, size=(bs, self.latentD))
        empty_list = []
        # for temp_i in range(self.latentD):
        #     temp_list = np.random.uniform(-1, 1, 100)
        #     temp_list.sort()
